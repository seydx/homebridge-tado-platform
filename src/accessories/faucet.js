...
class FaucetAccessory {
  ...
  getService () {
    ...
    if(!service){
      ...
      service = this.accessory.addService(this.api.hap.Service.Valve, this.accessory.displayName, this.accessory.context.config.subtype);
    }
    ...
    service.getCharacteristic(this.api.hap.Characteristic.Active)
      ...
      .on('change', value => {
        service.getCharacteristic(this.api.hap.Characteristic.InUse).updateValue(value.newValue);
      });
