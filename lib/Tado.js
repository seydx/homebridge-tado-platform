...
class Tado {
  ...
  setZoneOverlay(home_id, zone_id, power, temperature, termination, type, unit, method) {
    var config = {
      setting: {
        type: type,
      },
    ...
    if (power.toLowerCase() == 'on') {
      ...
      if (temperature) {
        ...
        if(unit === 0){
          ...
          config.setting.temperature.celsius = temperature.toString();
 
        } else {

          config.setting.temperature.fahrenheit = temperature.toString();
