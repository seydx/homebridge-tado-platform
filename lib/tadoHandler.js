      ...
      for(const zone of homeZones){
        ...
        for(const device of zone.devices){
          ...
          if(zone.type === 'HOT_WATER'){
            ...
            if(!this.canSetTemperature)
              this.canSetTemperature = await this.tado.getZoneCapabilities(this.config.homeID, zone.id);
            ...
            if(zone.deviceTypes.length > 1){
              ...
              if(deviceType === 'BU')
                deviceArray.push({
                  ...
                  type: this.canSetTemperature.canSetTemperature ? 'thermostat' : 'boiler'
                });
